...
# http://www.practicalclouds.com/content/guide/backup-mysqlsh

# 1.0 - initial, take a dump and upload it using backup_keepx 
# 2.0 - make compatible with version 2.0 boot, aws instead of s3cmd
# 2.0.1 - Correct crazy error calculating the unixtime!
	...
	case $1 in
		...
		-d|-database) shift
                        if [[ "$1" != "" ]]; then
                                ...
                                $logger "-b requires a mysql database name as argument!"
                                return 1
                        fi
                        ;;
		-p|-port) shift
                        if [[ "$1" != "" ]]; then
                                ...
                                $logger "-o requires an mysql port number as argument!"
                                return 1
                        fi
                        ;;
		-u|-dbuser) shift
                        if [[ "$1" != "" ]]; then
                                ...
                                $logger "-u requires a mysql user name as argument!"
                                return 1
                        fi
                        ;;
		-w|-dbpass) shift
...
if [[ "$DBUSER" == "" || "$DBPASS" == "" || "$DATABASE" == "" ]]; then
	$logger "You must enter a valid mysql database, username and password which have permission to access it."
	exit 1
fi

# Make a dump of requested database...
DUMPCOMMAND="mysqldump -u $DBUSER -p$DBPASS"
[[ "$PORT" != "" ]] && DUMPCOMMAND="$DUMPCOMMAND -P $PORT"
DUMPCOMMAND="$DUMPCOMMAND $DATABASE" 

$logger "Running mysqldump of database $DATABASE..."
$DUMPCOMMAND >/tmp/$FILE
if [[ "$?" != "0" ]]; then
	$logger "ERROR: The mysqldump of the database $DATABASE failed!"
	rm -f /tmp/$FILE
	exit 1
fi
